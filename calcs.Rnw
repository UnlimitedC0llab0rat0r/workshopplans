...
<<echo=T>>==
width_of_structure <- 15*12
width_of_structure_half <- width_of_structure/2
post_width <- 8
rafter_height <- 8
collar_tie_height <- 8
...
<<>>==
###
# NEW AFTER FIXING ERRORS
###

x1 <- width_of_structure_half - post_width - (kp_width/2)
...
kp_height_to_spring_line_excl_top_tenon <- kp_part + o2 + x4 # wrong because x4
...
x_3 &= h_2 - \text{post width} = \Sexpr{x3}\\
x_2 &= x_1 - x_3 = \Sexpr{x2}\\
h_3 &= x_2/\cos\theta = \Sexpr{h3}\\
x_4 &= \sqrt{h_3^2 - x_2^2} = \Sexpr{x4}
\end{align*}
...
h_4 &= (h_1 + \text{post width} + x_1 + \text{kingpost width}/2)/\cos\theta = \Sexpr{h4}\\
o_1 &= \sqrt{h_4^2 - (h_1 + \text{post width} + x_1 + \text{kingpost width}/2)^2}  = \Sexpr{o1}
\end{align*}

Calculating $o_1$ a different way: add up ridge height, kp part, $o_2$, and $x_4$, to get \Sexpr{o1_alt}, which should match the above. For diagram 2, we have $x_5 = x_1 - x_2 =$ \Sexpr{x5} $ \implies h_5 =x_5/\cos\theta = $ \Sexpr{h5}, which is enough space between post and collar tie. Since KP from top (excluding tenon) to spring line is \Sexpr{kp_height_to_spring_line_excl_top_tenon}, and bottom of collar tie is at the spring line, KP length excluding \textbf{both} tenons is \Sexpr{kp_height_to_spring_line_excl_top_tenon} - \Sexpr{collar_tie_height} (collar tie height) = \Sexpr{kp_height_excl_tenons}. Assuming a \Sexpr{kp_through_tenon_length}-inch through tenon at bottom and \Sexpr{kp_top_tenon_length}-inch tenon at the top, we have 
...
\begin{itemize}
  ...
  \item ``top of ridge to spring line'' is comprised of ``kp height to spring line excl top tenon'' from \Cref{kp-collar-tie-calculations} and ``ridge height'' from \Cref{ridge-shape-and-dimensions-calculations}.
...
See \Cref{posts-plan} for measurements including tenons. 

The structure has an overhang on both sides, which we take to be \Sexpr{overhang_sides} (cf \Cref{posts-rafters-assumptions}), and this allows us to calculate $x_8$ in the diagram: 
\[ x_8 = \frac{24}{\cos\theta} = \Sexpr{x8}. \]
...
\begin{itemize}
  \item $\bigstar$ $b_2$ = \Sexpr{sw(b2)}
  \item $\bigstar$ $b_3$ = \Sexpr{sw(b3)}
  \item $\bigstar$ $b_8$ = \Sexpr{sw(b8)}
  \item $h_1$ = \Sexpr{sw(h1)}
  \item $h_2$ = \Sexpr{sw(h2)}
  \item $h_4$ = \Sexpr{sw(h4)}
  \item $k_3$ = \Sexpr{sw(k3)}
  \item $\bigstar$ $k_{17}$ = \Sexpr{sw(k17)}
  \item $\bigstar$ $s_{13}$ = \Sexpr{sw(s13)}
  \item $\bigstar$ $x_1$ = \Sexpr{sw(x1)}
  \item $x_2$ = \Sexpr{sw(x2)}
  \item $\bigstar$ $x_3$ = \Sexpr{sw(x3)}
  \item $\bigstar$ $x_6$ = \Sexpr{sw(x6)}
  \item $x_8$ = \Sexpr{sw(x8)}
  \item base to spring line = \Sexpr{sw(base_to_spring_line)}
  \item $\bigstar$ brace width = \Sexpr{sw(brace_width)}
  ...
  \item $\bigstar$ post width = \Sexpr{sw(post_width)}
  ...
  \item structure width = \Sexpr{sw(width_of_structure)}
  \item $\bigstar$ strut width = \Sexpr{sw(strut_width)}
