...
\begin{itemize}
  \item In ``Imported variables'' those which just get the imperial string for variables defined elsewhere aren't included. These aren't important enough to count as genuine imported variables. 
...
Set KP part to a reasonable amount (cf \Cref{ridge-shape-and-dimensions-assumptions}), and this dictates that
...
<<echo=T>>==
# The "structure" here is just the workshop part - shed part calculations are separate. The original width was 15 ft. 
structure_width_feet <- 15
structure_width <- structure_width_feet*12
structure_height_feet <- 15
structure_height <- structure_height_feet*12
...
@
Start with purlin height above rafters, and the width of the structure. Ignore collar tie for now.
...
h6 <- ((overhang_sides + (0.5 * structure_width - 0.5 * kp_width))/cos(theta)) - x8
x6 <- tan(theta) * x5
...
\begin{center}
	\includegraphics[width=0.75\textwidth]{images/kingpost_and_collar_tie_lengths_new}
\end{center}
...
x_8 &= \text{overhang}/\cos\theta = \Sexpr{x8}\\
h_6 &= \Sexpr{h6}\\
a_7 &= \Sexpr{a7}
\end{align*}
...
These can also be calculated for later:
\begin{align*}
...
\subsubsection{Does rafter line intersect spring point?} 

\begin{center}
	\includegraphics[width=0.55\textwidth]{images/rafter_intersect_sp}
\end{center}

It need not since it depends on the height of the rafter. The top rafter line will intersect the SP if and only if 
\[ \text{rafter height } = \phi  \]
<<>>==
phi <- sin(theta) * a8
does_phi_equal_rafter_height <- (round(phi, 4) == rafter_height)
@

Given current assumptions, 
\[ \phi = \sin\theta\times a_8 = \Sexpr{phi}   \]
but the value of $\phi$ of course depends on various factors, for example, $dpct$:
\begin{align*}
dpct = 5.853 \implies \phi = 8 \text{ (hardcoded value)}\\
dpct = 10 \implies \phi = 10.0395 \text{ (hardcoded value)}
\end{align*}
and so on. \textbf{The value of dpct was picked to be 5.853 so that rafter does indeed intersect the spring point.} This means that the spring point contains the intersection of post, collar tie, as well as rafter, which is nice and neat. 

\begin{center} \fbox{\textbf{CHECK: Does $\phi$ equal rafter height? \Sexpr{does_phi_equal_rafter_height}}} \end{center}
...
The variable for ``KP part'' was not labeled in \Cref{ridge-shape-and-dimensions-assumptions}; here it's labeled as $k_2$; its value is \Sexpr{kp_part}. The KP tenon height into ridge ($k_1$) is \Sexpr{kp_top_tenon_length} (cf \Cref{kp-rafter-joints-imported-variables}). The variable $k_3$ is just rafter height divided by the cosine, which is 
...
The variable $k_5$ is given by collar tie height + through tenon length; here, $k_5 = \Sexpr{k5}$. Then, 
...
Now, rafter length can be calculated more precisely than in \Cref{posts-rafters-calculations}, where tenon length was just estimated: 
...
\begin{itemize}
  ...
  \item \verb+dpct+ = \Sexpr{dpct} from \Cref{kp-collar-tie-calculations}.
...
First, calculate rafter length on underside, which is 
...
For post tenons into rafters at \Sexpr{post_tenon_into_rafter_thickness}-in thick, 
\begin{align*}
...
Since tenon depth = \Sexpr{collar_tie_tenon_into_rafter_depth}, $c_2$ = \Sexpr{c2}. Since KP width at bottom is \Sexpr{kp_width_bottom}, modify from above (\Cref{ridge-shape-and-dimensions-assumptions}) where KP width was \Sexpr{kp_width} (\Cref{ridge-shape-and-dimensions-assumptions}):
...
\section{Braces for bents} \label{braces}
...
\begin{center}\fbox{\textbf{TODO Does this use bad values, e.g. x5?}}\end{center}
...
\begin{wrapfigure}[25]{r}{0.45\textwidth}
  \begin{center}
  	\includegraphics[width=0.45\textwidth]{images/post_calculations}
  \end{center}
...
p5 <- post_tenon_depth_into_rafter/sin(theta1)
...
Some of the calculations were covered above in \Cref{rafter-lengths-and-connections} from the perspective of the mortise. Here, concentrate on the tenons. Some variables are imports from the section mentioned. For the others, 
\begin{align*}
  p_2 &= \sqrt{p_3^2 - \text{post width}^2} = \Sexpr{p2}\\
  p_4 &= \cos\theta\times\text{post tenon depth into rafter} = \Sexpr{p4}\\
  p_5 &= \text{post tenon depth into rafter}/\sin\theta_1 = \Sexpr{p5}
\end{align*}
...
Lengths:
\begin{enumerate}
  \item Post: \Sexpr{post_height_total_imperial} or \Sexpr{post_height_total}
  \item Rafter: \Sexpr{rafter_length_final_imperial} or \Sexpr{rafter_length_final}
  \item Kingpost: \Sexpr{kp_total_height_imperial} or \Sexpr{kp_total_height}
  \item Collar Tie: \Sexpr{collar_tie_length_imperial} or \Sexpr{collar_tie_length}
  \item Struts: \Sexpr{strut_length_imperial} or \Sexpr{strut_length}
  \item Braces: \Sexpr{brace_total_length_imperial} or \Sexpr{brace_total_length}
\end{enumerate}

\newpage

\subsection{Posts (scale 1/10th inches to inches)}
\begin{center} \fbox{\textbf{TODO need to account for girt brace too.}} \end{center}
\begin{center} \fbox{\textbf{TODO do scale diagrams for all members.}} \end{center}
...
p5_imperial <- get_imperial_string(p5)
r6_imperial <- get_imperial_string(r6)
post_stub_tenon_width <- 2
post_stub_tenon_depth <- 2
post_scale <- function(x) {
  get_imperial_string(round(x/10, 2))
}
post_height_total_scaled <- post_scale(post_height_total)
p4_scaled_post <- post_scale(p4)
b3_scaled_post <- post_scale(b3)
b15_scaled_post <- post_scale(b15)
post_rest_minus_stub_scaled_post <- post_scale(post_rest_minus_stub)
p3_scaled_post <- post_scale(p3)
p5_scaled_post <- post_scale(p5)
p1_scaled_post <- post_scale(p1)
r13_scaled_post <- post_scale(r13)
r6_scaled_post <- post_scale(r6)
b13_scaled_post <- post_scale(b13)
b14_scaled_post <- post_scale(b14)
post_width_scaled_post <- post_scale(post_width)
post_depth_scaled_post <- post_scale(post_depth)
post_stub_tenon_length_scaled_post <- post_scale(post_stub_tenon_length)
post_tenon_depth_into_rafter_scaled_post <- post_scale(post_tenon_depth_into_rafter)
post_rafter_shoulder_depth_scaled_post <- post_scale(post_rafter_shoulder_depth)
brace_tenon_depth_into_post_scaled_post <- post_scale(brace_tenon_depth_into_post)
@
Layout \& cut steps (scale diagram measurements marked \textbf{S}): 
\begin{enumerate}
  \item Cut a \Sexpr{post_depth}$\times$\Sexpr{post_width} beam to \Sexpr{post_height_total_imperial} (\textbf{S}: \Sexpr{post_depth_scaled_post}$\times$\Sexpr{post_width_scaled_post}, \Sexpr{post_height_total_scaled}). 
  \item Mark out 
  \begin{itemize}
    \item $p_4$ = \Sexpr{p4_imperial} (\textbf{S}: \Sexpr{p4_scaled_post})
    \item $b_3$ = \Sexpr{b3_imperial} (\textbf{S}: \Sexpr{b3_scaled_post})
    \item $b_{15}$ = \Sexpr{b15_imperial} (\textbf{S}: \Sexpr{b15_scaled_post}), then 
    \item the rest minus stub tenon = \Sexpr{post_rest_minus_stub_imperial} (\textbf{S}: \Sexpr{post_rest_minus_stub_scaled_post}), and 
    \item verify \Sexpr{post_stub_tenon_length} (\textbf{S}: \Sexpr{post_stub_tenon_length_scaled_post}) inches left over for stub tenon. 
  \end{itemize}
  \item At top, cut shouldered tenon as in diagram: 
  \begin{itemize}
    \item Mark out $\theta$ = \Sexpr{round(theta_degrees, 1)} as shown on the right side and mark out tenon depth = \Sexpr{post_tenon_depth_into_rafter} (\textbf{S}: \Sexpr{post_tenon_depth_into_rafter_scaled_post}).
    \item At $90^\circ$, measure out $r_{13}$ = \Sexpr{r13_imperial} (\textbf{S}: \Sexpr{r13_scaled_post}).
    \item Then $p_5$ down the side, which is \Sexpr{p5_imperial} (\textbf{S}: \Sexpr{p5_scaled_post}).
    \item Mark out $r_{13}$ = \Sexpr{r13_imperial} (\textbf{S}: \Sexpr{r13_scaled_post}) at tenon cheek and verify that tenon depth is consistent.
    \item Draw line down at $90^\circ$ and verify that $p_1$ = \Sexpr{p1_imperial} (\textbf{S}: \Sexpr{p1_scaled_post}).
    \item Mark out shoulder = \Sexpr{post_rafter_shoulder_depth} (\textbf{S}: \Sexpr{post_rafter_shoulder_depth_scaled_post}) and verify that $\theta_2$ = \Sexpr{round(theta2_degrees, 1)} and $r_6$ = \Sexpr{r6_imperial} (\textbf{S}: \Sexpr{r6_scaled_post})
    \item \textbf{this is the tenon cut line}.  
  ...
  \item Cut brace mortise:
    \begin{itemize}
      \item Mark out 
      \begin{itemize}
        \item $b_{13}$ = \Sexpr{b13_imperial} (\textbf{S}: \Sexpr{b13_scaled_post}) and 
        \item $b_{14}$ = \Sexpr{b14_imperial} (\textbf{S}: \Sexpr{b14_scaled_post}).
      \end{itemize}
      \item Drill straight at $b_{14}$ to depth of \Sexpr{brace_tenon_depth_into_post} (\textbf{S}: \Sexpr{brace_tenon_depth_into_post_scaled_post}).
      \item Angled mortise at $b_{13}$ at an angle of $\theta_1$ = \Sexpr{round(theta1_degrees, 1)}
    \end{itemize}
...
\newpage
\subsection{Rafters (scale 1/15 inches to inches)}\label{rafter-cut-plan}
...
rafter_scale <- function(x) {
  get_imperial_string(round(x/15, 2))
}
rafter_width_scaled_rafter <- rafter_scale(rafter_width)
rafter_height_scaled_rafter <- rafter_scale(rafter_height)
rafter_length_final_scaled_rafter <- rafter_scale(rafter_length_final)
@
...
<<>>==
x8_scaled_rafter <- rafter_scale(x8)
h6_scaled_rafter <- rafter_scale(h6)
k14_scaled_rafter <- rafter_scale(k14)
r7_scaled_rafter <- rafter_scale(r7)
r5_scaled_rafter <- rafter_scale(r5)
r4_scaled_rafter <- rafter_scale(r4)
r3_scaled_rafter <- rafter_scale(r3)
r2_scaled_rafter <- rafter_scale(r2)
left_over_rafter_scaled_rafter <- rafter_scale(left_over_rafter)
@
\begin{enumerate}
  \item Measure out \Sexpr{rafter_width}$\times$\Sexpr{rafter_height}$\times$\text{\Sexpr{rafter_length_final_imperial}} (\textbf{S}: \Sexpr{rafter_width_scaled_rafter} $\times$ \Sexpr{rafter_height_scaled_rafter} $\times$ \text{\Sexpr{rafter_length_final_scaled_rafter}}).
  \item On top side mark out (left to right) $x_8 = $ \Sexpr{x8_imperial} (\textbf{S}: \Sexpr{x8_scaled_rafter}) and $h_6 = $ \Sexpr{h6_imperial} (\textbf{S}: \Sexpr{h6_scaled_rafter}). Verify that $k_{14}$ = \Sexpr{k14_imperial} (\textbf{S}: \Sexpr{k14_scaled_rafter}). 
  \item On the bottom, mark out (left to right) the following:
  \begin{align*}
    r_7 &= \Sexpr{r7_imperial} (\textbf{S}: \Sexpr{r7_scaled_rafter})\\
    r_5 &= \Sexpr{r5_imperial} (\textbf{S}: \Sexpr{r5_scaled_rafter})\\
    r_4 &= \text{\Sexpr{r4_imperial}} (\textbf{S}: \Sexpr{r4_scaled_rafter})\\
    r_3 &= \Sexpr{r3_imperial} (\textbf{S}: \Sexpr{r3_scaled_rafter})\\
    r_2 &= \Sexpr{r2_imperial} (\textbf{S}: \Sexpr{r2_scaled_rafter})\\
    & \text{and verify that what's left over} = \text{\Sexpr{left_over_rafter_imperial}} (\textbf{S}: \Sexpr{left_over_rafter_scaled_rafter}).
  \end{align*}
\end{enumerate}
...
\end{center}
<<>>==
post_rafter_shoulder_depth_scaled_rafter <- rafter_scale(post_rafter_shoulder_depth)
collar_tie_rafter_shoulder_depth_scaled_rafter <- rafter_scale(collar_tie_rafter_shoulder_depth)
r12_imperial_rafter <- get_imperial_string(r12)
post_tenon_depth_into_rafter_scaled_rafter <- rafter_scale(post_tenon_depth_into_rafter)
r6_scaled_rafter <- rafter_scale(r6)
r12_scaled_rafter <- rafter_scale(r12)
r13_scaled_rafter <- rafter_scale(r13)
@
At $r_5$, mark out \Sexpr{post_rafter_shoulder_depth}-in shoulder (\textbf{S}: \Sexpr{post_rafter_shoulder_depth_scaled_rafter}), verify that $\theta_2$ = \Sexpr{round(theta2_degrees, 1)} and
\begin{align*}
\text{mortise depth} &= \Sexpr{post_tenon_depth_into_rafter} (\textbf{S}: \Sexpr{post_tenon_depth_into_rafter_scaled_rafter})\\
r_6 &= \Sexpr{r6_imperial} (\textbf{S}: \Sexpr{r6_scaled_rafter})\\
r_{12} &= \Sexpr{r12_imperial_rafter} (\textbf{S}: \Sexpr{r12_scaled_rafter})\\
r_{13} &= \Sexpr{r13_imperial} (\textbf{S}: \Sexpr{r13_scaled_rafter})
\end{align*}
...
\end{center}
<<>>==
r10_imperial <- get_imperial_string(r10)
collar_tie_tenon_into_rafter_depth_scaled_rafter <- rafter_scale(collar_tie_tenon_into_rafter_depth)
r9_scaled_rafter <- rafter_scale(r9)
r10_scaled_rafter <- rafter_scale(r10)
r11_scaled_rafter <- rafter_scale(r11)
r9_imperial <- get_imperial_string(r9)
r11_imperial <- get_imperial_string(r11)
@
At $r_3$, mark out \Sexpr{collar_tie_rafter_shoulder_depth}-in shoulder (\textbf{S}: \Sexpr{collar_tie_rafter_shoulder_depth_scaled_rafter}) and verify that $\theta_3$ = \Sexpr{round(theta3_degrees, 1)} and 
\begin{align*}
\text{mortise depth} &= \Sexpr{collar_tie_tenon_into_rafter_depth} (\textbf{S}: \Sexpr{collar_tie_tenon_into_rafter_depth_scaled_rafter})\\
r_9 &= \Sexpr{r9_imperial} (\textbf{S}: \Sexpr{r9_scaled_rafter})\\
r_{10} &= \Sexpr{r10_imperial} (\textbf{S}: \Sexpr{r10_scaled_rafter})\\
r_{11} &= \Sexpr{r11_imperial} (\textbf{S}: \Sexpr{r11_scaled_rafter})
\end{align*}

\begin{center} \fbox{\textbf{Note that the measurements above were not quite exact in scale diagram.}} \end{center}
...
<<>>==
rafter_tenon_depth_into_kp_scaled_rafter <- rafter_scale(rafter_tenon_depth_into_kp)
k10_scaled_rafter <- rafter_scale(k10)
k13_scaled_rafter <- rafter_scale(k13)
k7_scaled_rafter <- rafter_scale(k7)
k3_scaled_rafter <- rafter_scale(k3)
rafter_tenon_thickness_scaled_rafter <- rafter_scale(rafter_tenon_thickness)
@

\begin{enumerate}
  \item $k_{14}$ = \text{\Sexpr{k14_imperial}} (\textbf{S}: \Sexpr{k14_scaled_rafter}) should have been marked out already. 
  \item Mark out $\theta_6$ = \Sexpr{round(theta6_degrees,1)} and verify rafter tenon depth into KP (\text{\Sexpr{rafter_tenon_depth_into_kp_imperial}} (\textbf{S}: \Sexpr{rafter_tenon_depth_into_kp_scaled_rafter})).
  \item Mark $90^\circ$ (\textbf{this angle is hardcoded as 90}).
  \item Verify $k_{10} = $  \text{\Sexpr{k10_imperial}} (\textbf{S}: \Sexpr{k10_scaled_rafter}).
  \item Measure $k_{13} = $ \text{\Sexpr{k13_imperial}} (\textbf{S}: \Sexpr{k13_scaled_rafter}).
  \item Measure $k_7 = $ \text{\Sexpr{k7_imperial}} (\textbf{S}: \Sexpr{k7_scaled_rafter}).
  \item Verify that this mark coincides with the ``leftover'' mark made above. 
  \item Mark up to $k_{14}$ point.
  \item Verify $k_3 = $ \text{\Sexpr{k3_imperial}} (\textbf{S}: \Sexpr{k3_scaled_rafter}).
  \item Verify tenon depth is correct all along tenon. 
...
\end{center}
<<>>=
s3_plus_s8_scaled_rafter <- rafter_scale(s3_plus_s8)
s9_scaled_rafter <- rafter_scale(s9)
strut_width_scaled_rafter <- rafter_scale(strut_width)
strut_tenon_into_rafter_depth_rafter <- rafter_scale(strut_tenon_into_rafter_depth)
@
For strut mortises into rafter, measure $s_3 + s_8$ from end of collar tie mortise, which is 
\[ s_3 + s_8 = \Sexpr{s3_plus_s8} = \Sexpr{s3_plus_s8_imperial} (\textbf{S}: \Sexpr{s3_plus_s8_scaled_rafter}).\]

Mortise/tenon is \Sexpr{strut_tenon_into_rafter_thickness}-in thick, \Sexpr{strut_width}-in wide (\textbf{S}: \Sexpr{strut_width_scaled_rafter}), and \Sexpr{strut_tenon_into_rafter_depth}-in deep (\textbf{S}: \Sexpr{strut_tenon_into_rafter_depth_rafter}). Verify that $s_9$ = \Sexpr{s9_imperial} (\textbf{S}: \Sexpr{s9_scaled_rafter}). 


\textbf{TODO rafters will house purlins - account for those also when purlin nos is finalized.}

\begin{center} \fbox{\textbf{TODO The tenon depths are quite large; they can probably be 3 inches, no need for 4.}} \end{center}
...
\section{Scale diagram: structure width = 15 ft, height = 15 ft}

\begin{center}
	\includegraphics[width=0.85\textwidth]{images/scale_diagram}
\end{center}

{\scriptsize May not be final. Other restrictions apply.}


\newpage

\section{Scale diagram: structure width = 10 ft, height = 13 ft}

\textbf{This is too small, probably not doing this.}
\begin{center}
	\includegraphics[width=0.85\textwidth]{images/scale_diagram_width10ft_height13ft}
\end{center}
